...
def open_compressed_files(dir:str, n:int) -> list:
    """
    Open the compressed files from a directory and save them in a list.
    ...
    try:
        ...
        for filename in os.listdir(dir): # Open every filename from the directory.
            ...
            with open(rpath, 'rb') as file: # Open the file.
                ...
                compressed_data.append((data, filename)) # Append to the return list.

            cont += 1
        return compressed_data # Return the data list.
...
def create_ncd_mixed_matrix(dir1_path:str, dir1_qtt:int, dir2_path:str, dir2_qtt:int, output_path:str, compressor:str):
    ...
    """
    dir1_path = os.path.join(dir1_path, compressor)
    dir2_path = os.path.join(dir2_path, compressor)

    try:
        print(f"Opening {dir1_qtt} compressed files from {dir1_path}...")
        data = open_compressed_files(dir1_path, dir1_qtt) # Open the first set of compressed files
        
        print(f"Opening {dir2_qtt} compressed files from {dir2_path}..")
        data.extend(open_compressed_files(dir2_path, dir2_qtt)) # Append the second set of compressed files to the datalist.
    ...
    with open(sorted_path, mode='w', newline='') as file:
        ...
        for item in sorted_data:
            linha_formatada = [item[0], item[1], f"{item[2]:.5f}"]
            writer.writerow(linha_formatada)